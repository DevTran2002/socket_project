...
class ServerWorker:
        ...
        def Recv_RtspRequest(self):
            while(1):
                ...
                if data:
                    ...
                    check = self.Process_RtspRequest(data.decode('utf-8'))
                    if check==1:
                            break
        ...
        def sendRtp(self):
            ...
            while(1):
                ...
                if self.event.isSet():
                    break
                (check,data) = self.Stream.NewnextFrame()
                if check == True:
                        if data:
                            ...
                            try:
                                ...
                                if prb > 5.0:
                                        ...
                                        self.handle_bigdata(data,int(15000),frameNumber,port)
                                        counter+=1
                                        time.sleep(jit)
                            except:
                                traceback.print_exc(file=sys.stdout)
                                self.Response_Rtsp(self.EOF,0)
                                self.Stream.release()
                        ...
                        self.Response_Rtsp(self.EOF,0)
                        self.Stream.release()
                        print("Got an error with NewnextFrame function")
                        break
        ...
        def Response_Rtsp(self,code,seq):
                if code == self.OK_200:
                        ...
                        reply = 'BREAK'
                        self.rtsp_socket.send(reply.encode('utf-8'))
        def handle_bigdata(self,data,maxsize,frameNbr,port):
            length = int(len(data))
            print("Length of raw_data: "+str(length))
            if length>maxsize:
                size = int(length/maxsize)
                for i in range(0,size):
                    try:
                        self.rtp_socket.sendto(self.makeRtp(data[int(i*maxsize):int(i*maxsize+maxsize)],frameNbr,i+1,0),(self.client_address[0],port))
                        #time.sleep(0.08)
                    except:
                        traceback.print_exc(file=sys.stdout)
                final_length = int(length - size*maxsize)
                self.rtp_socket.sendto(self.makeRtp(data[int(size*maxsize):int(size*maxsize+final_length)],frameNbr,size+1,1),(self.client_address[0],port))
            else:
                print("Check")
                try:
